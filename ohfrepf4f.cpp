...
public:
    ...
    void addEdge(int src, int dest, int w) {
        ...
        while (temp) {
            if (temp->data == dest) {
                cout << "\nEdge already exists between " << src << " and " << dest << ".\n";
                return;
            }
            temp = temp->next;
        }
    ...
    void dijkstra(int src) {
        int dist[totalnodes + 1];  // Distance array
        bool visited[totalnodes + 1];  // Visited array
        ...
        // Main loop to calculate shortest distances
        for (int count = 1; count <= totalnodes; count++) {
            ...
            if (u == -1) break;  // All vertices are visited or unreachable
